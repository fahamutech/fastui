...
function rebaseV2Paths(value, sourceSpecPath, destinationSpecPath, key = '') {
    if (Array.isArray(value)) return value.map(item => rebaseV2Paths(item, sourceSpecPath, destinationSpecPath));
    if (!value || typeof value !== 'object') {
        return ['ref', 'compose'].includes(key)
            ? rebaseLocalPath(value, sourceSpecPath, destinationSpecPath)
            : value;
    }
    return Object.fromEntries(Object.entries(value).map(([childKey, child]) => [
        childKey,
        rebaseV2Paths(child, sourceSpecPath, destinationSpecPath, childKey)
    ]));
}
...
async function resolveSpecDocument(specPath, stack = []) {
    ...
    if (`${document.version ?? ''}`.startsWith('fastui/') && document.ref) {
        const referencePath = pathResolve(dirname(absolutePath), document.ref);
        const referenceDocument = await resolveSpecDocument(referencePath, [...stack, absolutePath]);
        const referenceData = rebaseV2Paths(referenceDocument, referencePath, absolutePath);
        const override = structuredClone(document);
        delete override.ref;
        return deepMerge(referenceData, override);
    }
