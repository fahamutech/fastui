export function flutterRuntimeSourceV2() {
...
class FastUISurfaceDefinition {
  const FastUISurfaceDefinition({required this.type, required this.builder});
  final String type;
...
class FastUIScrollableSurface extends StatelessWidget {
  const FastUIScrollableSurface({super.key, required this.child});
...
class FastUIRouterDelegate extends RouterDelegate<String>
  ...
  Widget build(BuildContext context) => Navigator(
    ...
    pages: _pages.map((name) => MaterialPage<void>(
      ...
      name: '/\$name',
      child: FastUIScrollableSurface(child: FastUINavigation.surface(name)),
    )).toList(),
    onDidRemovePage: (_) {},
  );
}
...
class FastUINavigation {
  ...
  static Future<T?> _navigate<T>({
    ...
    final child = PopScope(
      ...
      child: FastUIScrollableSurface(child: surface(route.name!)),
    );
    Future<T?> result;
    if (type == 'sheet') {
      ...
      result = showGeneralDialog<T>(
        ...
        transitionBuilder: (dialogContext, animation, secondaryAnimation, child) => _transition(animation, child, transition, direction),
      );
    }
