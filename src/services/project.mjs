...
import {getBlueprintRoot, normalizeTemplate} from '../utils/config.mjs';
import {
...
import {ensurePrimitiveSpecs, v2Schema} from '../schema/primitives.mjs';
...
export async function initializeProject({template, runCommand} = {}) {
    ...
    await writeFile(resolve('fastui.v2.schema.json'), JSON.stringify(v2Schema, null, 2));
    await ensurePrimitiveSpecs({blueprintRoot, writeIfMissing});
